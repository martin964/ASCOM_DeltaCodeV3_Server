//
// ASCOM.DeltaCodeV3 Local COM Server
//
...
namespace ASCOM.DeltaCodeV3
{
	public static class Server
	{
		...
		private static string s_appId = "{1EAC6272-7F7D-4428-B1F0-88949AE80601}";   // Our AppId
			...
			foreach (FileInfo fi in d.GetFiles("*.dll"))
			{
                ...
                catch (Exception e)
				{
					MessageBox.Show("Failed to load served COM class assembly " + fi.Name + " - " + e.Message,
						"DeltaCodeV3", MessageBoxButtons.OK, MessageBoxIcon.Stop);
					return false;
				}
        ...
        private static void ElevateSelf(string arg)
        {
            ...
            {
                MessageBox.Show("The DeltaCodeV3 was not " + (arg == "/register" ? "registered" : "unregistered") +
                    " because you did not allow it.", "DeltaCodeV3", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString(), "DeltaCodeV3", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            return;
        }
				...
				switch (args[0].ToLower())
				{
					...
					default:
						MessageBox.Show("Unknown argument: " + args[0] + "\nValid are : -register, -unregister and -embedding",
							"DeltaCodeV3", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
						break;
				}
			}
			else
